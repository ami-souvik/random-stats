...
import {
  ...
  setBulkSchema,
  Question,
} from '@/src/store/slice/dataSlice';
...
const Select = ({ label, name, control, options }: SelectProps) => (
  <div className="space-y-1.5">
    {label && <label className="section-label block">{label}</label>}
    <Controller
      name={name}
...
const SortableHeader = ({ q }: { q: Question }) => {
  ...
  return (
    <th
      ref={setNodeRef}
      style={style}
      className="px-4 py-3 font-semibold text-white/40 uppercase tracking-wider whitespace-nowrap relative group text-left"
    >
      <div className="flex items-center gap-2">
        <div
          {...attributes}
          {...listeners}
          className="cursor-grab active:cursor-grabbing text-white/10 group-hover:text-white/30 transition-colors"
        >
...
const SortableSchemaItem = ({
  ...
  return (
    <motion.div
      ...
      style={style}
      layout
      className={`group flex items-center justify-between p-2 rounded-lg transition-all border ${isEditing ? 'bg-white/10 border-white/20' : 'bg-white/[0.02] border-white/5 hover:border-white/10'}`}
    >
      <div
...
function HomeContent() {
  ...
  const questions = useSelector((state: RootState) => state.data.questions);
  ...
  useEffect(() => {
    ...
    if (s || c) {
      try {
        let decodedQuestions: Question[] | null = null;
        if (s) {
  ...
  const onAddOrUpdateField = (data: any) => {
    ...
    if (data.type === 'choices')
      ...
      field.min = Number(data.min);
      field.max = Number(data.max);
      field.multiplier = Number(data.multiplier);
              ...
              <div className="h-full flex flex-col items-center justify-center space-y-4 grayscale opacity-40">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-white/40 mt-1">
                    Schema required for sequence initialization.
                  </p>
                </div>
              </div>
            )}
...
export default function Home() {
  return (
    <Suspense
      fallback={
        <div className="h-screen bg-black flex items-center justify-center font-serif italic text-white/40">
          Loading Engine...
        </div>
      }
    >
